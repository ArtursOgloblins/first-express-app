...
describe('tests for blogs and posts likes', () => {
    ...
    it('add like to post by user one', async () => {
        await request(app)
            .put(`${RouterPath.posts}/${newPost.id}/like-status`)
    ...
    it('get post by id', async () => {
        const response = await request(app)
            .get(`${RouterPath.posts}/${newPost.id}`)
            .set('Authorization', `Bearer ${userOneToken}`)
                ...
                newestLikes: [
                    {
                        ...
                        login: userOneData.login
                    }]
            }
        })
    })

    it('add like to comment by user one', async () => {
        await request(app)
            .put(`${RouterPath.comments}/${newComment._id}/like-status`)
            .set('Authorization', `Bearer ${userOneToken}`)
            .send({likeStatus: "Dislike"})
            .expect(HTTP_STATUS.NO_CONTENT)
    })

    it('add dislike to comment by user two', async () => {
        await request(app)
            .put(`${RouterPath.comments}/${newComment._id}/like-status`)
            .set('Authorization', `Bearer ${userTwoToken}`)
            .send({likeStatus: "Dislike"})
